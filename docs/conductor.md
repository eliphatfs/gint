...
- `gint.conductor.compile(fn, *, dynamo_cache_size_limit=N)` is an opt-in escape hatch for shape-cycling workloads (e.g. mip-level loops that hit many distinct shapes): it scopes a larger `cache_size_limit` to the wrapped call only — via the same `torch._dynamo.config.patch` block that disables dynamic shapes — so no global config flip is needed. Left at the torch default when `None`: more cache entries also mean more guard checks per call, which measurably hurts small, hot compiled functions, so it is opt-in rather than on by default.
...
- The multi-batch 2dt/2dw selection is gated on `len(batch_dims) >= 1`. Set the env var `GINT_PW_1D_ONLY=1` to force the flat 1d tile regardless of `block_size` (skips `_select_pointwise_tiling`); useful as an A/B knob when diagnosing a regression, or for shapes where the multi-batch tile underperforms the coalesced 1d path.
...
- Complementary codegen-time reorder (fused reduction+broadcast+pointwise path only): for a commutative binary op where one arg already sits on top of the virtual stack and the other doesn't, `handle_operand` claims the stack-resident arg first and pushes the other on top. The declared arg order would otherwise bury the stack-resident arg under the fresh push and force a Swap or a dead global reload. Safe because commutativity makes the result order-invariant.
...
- `_plan_spills(nodes, ext_io)` walks the schedule and assigns a virtual register to two classes of node:
  ...
  2. **Buried single-use nodes** discovered by simulating the abstract stack in the codegen's actual `arg_order` and watching for any internal arg that lands at depth ≥ 2 by the time its `handle_operand` fires. Codegen has no rotN — only `swap` rescues depth 1 — so a buried single-use value would otherwise raise the "buried at depth ≥ 2" runtime error in `StackCodegen.handle_operand`. Burial detection iterates: each round adds the newly-buried args to the spill set and re-simulates, converging in O(nodes) rounds since each round adds ≥ 1 node or terminates. The simulation must keep `ext_io` nodes with internal uses on the abstract stack permanently (until the subgraph ends) — `emit_op`'s `dup + store` for an ext_io result deposits a copy at the bottom that the codegen never pops, and ignoring that copy under-reports stack depth and silently misses burial of innocent single-use neighbours (e.g. `cos` in ggx_importance fwd, buried by accumulated `mul`/`sqrt`/`sqrt_1` ext_io copies).
...
- `handle_operand`'s dup rule counts physical copies of an arg on the virtual stack against how many claims the current op has already made on it, dup'ing only when this claim has no unclaimed copy to take (e.g. the second `x` of `mul(x, x)` after a fresh load) or when consuming the last unclaimed copy would starve a later use. The previous `already_claimed >= 1 or uses_left > 1` rule double-dup'd `mul(x, x)` on an in-subgraph `x`, leaving a dead copy on the hardware stack for the rest of the program. The same copies-vs-claims accounting is mirrored in `_plan_spills`'s abstract-stack simulation so burial detection stays consistent with real codegen. At depth 1, a stack-resident copy on its last use is consumed in place via `Swap` (1 insn, no load) rather than reloaded from reg/global — reloading would leave the buried copy dead on the stack forever, wasting both a slot and a load.
- **Subgraph-fallback robustness**: stack-overflow (`_check_overflow`) and the unresolvable-stack `handle_operand` / fused-reduction-chunk errors ("not on stack", "buried at depth ≥ 2 with no global tensor entry") raise `_UnplannableSubgraph`, **not** `RuntimeError`. The compiler catches it per-subgraph and drops just that subgraph to the eager-fallback path instead of failing the whole AOT frame — so a single pathological subgraph (e.g. one whose shape the spill planner's simulation diverged from) degrades gracefully rather than aborting the entire compiled function. `_compile_reduction_subgraph` is wrapped the same way inside the fused-reduction merge loop.
...
- **Metadata-only subgraphs are skipped** (`_is_metadata_only` in the merger loop): if every node in a partitioned subgraph is `OpKind.METADATA`, we drop the schedule entirely so its nodes flow through `_run_eager`. Compiling such a subgraph would emit a redundant load+store kernel, while eager handles metadata for free (zero kernels). The shape-aware partitioner (output-shape-equality rule) tends to *create* these isolated metadata subgraphs whenever a metadata op sits between two pointwise chains with incompatible iteration shapes — `_sample2d_internal` and `_interpolate_impl` each produced one such 1-node subgraph per call.
