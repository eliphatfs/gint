...
- `gint/conductor/backend.py`: backend registration (`"gint"` default with CUDA graphs, `"gint-no-cuda-graph"` legacy alias). Also exposes `gint.conductor.compile` — a `torch.compile` drop-in that scopes `automatic_dynamic_shapes=False` to the wrapped call (no global config flip; required because gint compiles per-shape and can't accept SymInt FakeTensors). The CG path uses an inference-only `_direct_cudagraph_wrap` (not `make_graphed_callables`); options: `cuda_graphs`, `num_warmup_iters`, `clone_outputs` (default True for safe multi-frame held-output use; set False for max single-kernel perf). `compile(..., dynamo_cache_size_limit=N)` is an opt-in escape hatch for shape-cycling workloads that scopes a larger dynamo `cache_size_limit` to the wrapped call only (left at torch default when None — more entries also means more guard checks per call).
- `gint/conductor/compiler.py`: FX graph → bytecode conversion, graph partitioning, broadcasting. Stack-overflow / unresolvable-stack errors raise `_UnplannableSubgraph` so the compiler drops just that subgraph to eager instead of failing the whole AOT frame. Env var `GINT_PW_1D_ONLY=1` forces the flat 1d pointwise tile (skips the small-block 2dt/2dw selection).
