...
- **Geometry_smith fused-reduction** at N=3 (mode 2dw, pre_prefix=[mul], no suffix): `grid=1M → 32K`, wall 0.70 ms → 0.16 ms at M=1M. **Small-N RMSNorm-shape** at N=8 or N=64 (mode 2dw / 2dt, pre_prefix=[mul] + scalar_prefix=[+eps, rsqrt] + broadcast_suffix=[*x, *w]): now produces a single fused kernel covered by `tests/test_conductor_real_workloads.py::test_rms_norm_small_N_2dw / _2dt`
